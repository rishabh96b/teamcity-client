		...
		if m.AgentRequirement[i] != nil {
			if err := m.AgentRequirement[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					...
					return ce.ValidateName("agent-requirement" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
// ContextValidate validate this agent requirements based on the context it is used
func (m *AgentRequirements) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if err := m.contextValidateAgentRequirement(ctx, formats); err != nil {
		res = append(res, err)
	}
...
func (m *AgentRequirements) contextValidateAgentRequirement(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.AgentRequirement); i++ {

		if m.AgentRequirement[i] != nil {
			if err := m.AgentRequirement[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("agent-requirement" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("agent-requirement" + "." + strconv.Itoa(i))
